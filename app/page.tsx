...
export default function Home() {
  ...
  // اكتشاف المتصفح الداخلي لتطبيقات التواصل الاجتماعي
  ...
  return (
    <main
      ...
      dir="rtl"
    >
      {/* 🛑 الشريط الذكي المتنقل في تيك توك وإنستغرام */}
      {isInAppBrowser && (
        <div className="bg-red-600/95 backdrop-blur-md text-white text-center py-3 px-4 text-sm md:text-base font-bold sticky top-0 z-[100] shadow-2xl border-b-2 border-red-400 flex flex-col items-center justify-center animate-pulse">
          ...
          <span className="font-normal text-xs md:text-sm mt-1">
            لتعمل أزرار (الواتساب والاتصال) بشكل صحيح، يرجى الضغط على{" "}
            <strong className="bg-white/20 px-1 rounded">الثلاث نقاط ⠇</strong>{" "}
            بالأعلى واختيار <strong className="bg-white/20 px-1 rounded">فتح في المتصفح</strong>
          </span>
        </div>
      )}
      ...
      <section className="relative flex flex-col items-center justify-center text-center min-h-[95vh] px-4">
        <div className="absolute inset-0 z-0">
          <img src="/images/1.webp" alt="كارستايل لرش وزينة السيارات في الرياض" className="w-full h-full object-cover brightness-50" />
        ...
        <div className="relative z-10 max-w-5xl mt-10">
          ...
          <div className="flex flex-wrap justify-center gap-6">
            <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="bg-blue-600 hover:bg-blue-500 text-white px-12 py-4 rounded-full font-bold text-lg shadow-xl hover:-translate-y-1 transition-all w-full sm:w-auto text-center">
              حجز موعد
            </a>
            <a href={tiktokLink} target="_blank" rel="noopener noreferrer" className="bg-white/10 backdrop-blur-md border border-white/20 text-white px-12 py-4 rounded-full font-bold text-lg hover:bg-white/20 transition-all w-full sm:w-auto text-center">
              أعمالنا
            </a>
            <a href={phoneLink} className="bg-gray-100 hover:bg-white text-gray-900 px-12 py-4 rounded-full font-bold text-lg shadow-xl hover:-translate-y-1 transition-all w-full sm:w-auto text-center">
              اتصل بنا
            </a>
          </div>
        </div>
      ...
      {/* قسم الخدمات المتكامل (19 خدمة كاملة) */}
          ...
          {[
            ...
            <div key={s.id} className="group bg-gray-800/40 backdrop-blur-sm rounded-[2rem] overflow-hidden border border-gray-700 hover:border-blue-500 transition-all duration-500 shadow-2xl">
              <div className="h-64 overflow-hidden">
                <img src={`/images/${s.id}.webp`} alt={`${s.title} في الرياض - كارستايل`} className="w-full h-full object-cover group-hover:scale-110 transition-all duration-700" />
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[1, 2, 3].map((v) => (
              <div key={v} className="relative h-[500px] bg-gray-800 rounded-3xl overflow-hidden border border-gray-700 group">
                <video className="w-full h-full object-cover opacity-100 brightness-110 contrast-105" autoPlay muted loop playsInline>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {[
              ...
              <div key={i} className="bg-gray-800/70 border border-gray-700 rounded-3xl p-8 shadow-2xl hover:border-blue-500 transition-all">
          ...
          <div className="text-center mt-14 flex flex-wrap justify-center items-center gap-6">
            <a href={googleReviewLink} target="_blank" rel="noopener noreferrer" className="bg-blue-600 hover:bg-blue-500 text-white px-10 py-5 rounded-full font-bold text-lg shadow-xl hover:-translate-y-1 transition-all w-full sm:w-auto text-center">
              اضغط هنا لتقييمنا على قوقل ⭐
            </a>
            <a href={googleMapsLink} target="_blank" rel="noopener noreferrer" className="bg-white/10 backdrop-blur-md border border-white/20 text-white px-10 py-5 rounded-full font-bold text-lg hover:bg-white/20 transition-all w-full sm:w-auto text-center">
      ...
      {/* الفوتر المتكامل مع أوقات العمل والخريطة التفاعلية الصحيحة بالإبرة */}
      <footer className="relative py-24 px-4 overflow-hidden text-center z-10 border-t border-gray-800">
        <div className="absolute inset-0 z-0">
          <img src="/images/5.webp" alt="موقع كارستايل في الرياض" className="w-full h-full object-cover opacity-30 brightness-50" />
        ...
        <div className="relative z-10 max-w-6xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16 items-start">
            ...
            {/* قسم الخريطة التفاعلية بالإبرة المستهدفة */}
            <div className="flex flex-col items-center justify-center w-full">
              ...
              <div className="relative w-full max-w-md h-64 bg-gray-800 rounded-2xl overflow-hidden border border-gray-700 shadow-2xl group">
                ...
                {/* الخريطة الذكية المستهدفة التي تضمن ظهور الإبرة فوق الورشة بالغرابي مباشرة */}
                <iframe
