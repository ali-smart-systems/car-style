...
export default function Home() {
  ...
  const harajLink = "https://haraj.com.sa/users/طلاء_السيارات_الرياض_الغرابي/rate";

  // 1️⃣ 📍 الرابط المباشر لفتح موقع المحل في تطبيق الخرائط (ضعه بين علامتي الاقتباس)
  const googleMapsLink = "ضع_رابط_المشاركة_الذي_نسخته_هنا";

  // 2️⃣ 🗺️ رابط التضمين (Embed) لإظهار الخريطة التفاعلية والدبوس الأحمر داخل الفوتر
  const iframeSrc = "ضع_رابط_التضمين_src_الخاص_بالمحل_هنا";

  // اكتشاف المتصفح الداخلي (تيك توك، إنستغرام إلخ)
  ...
  return (
    <main className="min-h-screen bg-gray-900 text-gray-50 font-sans relative overflow-hidden text-right" dir="rtl">
      ...
      {/* 🛑 شريط التنبيه الذكي للمتصفحات الداخلية */}
      {isInAppBrowser && (
        <div className="bg-red-600/95 backdrop-blur-md text-white text-center py-3 px-4 text-sm md:text-base font-bold sticky top-0 z-[100] shadow-2xl border-b-2 border-red-400 flex flex-col items-center justify-center animate-pulse">
          ...
          <span className="font-normal text-xs md:text-sm mt-1">
            لتعمل أزرار الاتصال والموقع بكفاءة، يرجى الضغط على <strong className="bg-white/20 px-1 rounded">الثلاث نقاط ⠇</strong> بالأعلى واختيار <strong className="bg-white/20 px-1 rounded">فتح في المتصفح</strong>
          </span>
        </div>
      )}

      {/* الأزرار العائمة الثابتة على الشاشة */}
      <div className="fixed bottom-6 right-6 flex flex-col gap-4 z-50">
        ...
        {/* الاتصال */}
        <a href={phoneLink} className="bg-blue-600 hover:bg-blue-500 text-white p-4 rounded-full shadow-lg hover:scale-110 transition duration-300 flex items-center justify-center" title="اتصل بنا">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"></path></svg>
        </a>

        {/* واتساب */}
        <a href={whatsappLink} target="_blank" rel="noopener noreferrer" className="bg-green-500 hover:bg-green-400 text-white p-4 rounded-full shadow-lg hover:scale-110 transition duration-300 flex items-center justify-center" title="واتساب">
          <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 11.5a8.38 8.38 0 0 1-.9 3.8 8.5 8.5 0 0 1-7.6 4.7 8.38 8.38 0 0 1-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 0 1-.9-3.8 8.5 8.5 0 0 1 4.7-7.6 8.38 8.38 0 0 1 3.8-.9h.5a8.48 8.48 0 0 1 8 8v.5z"></path></svg>
        </a>

        {/* حراج */}
        <a href={harajLink} target="_blank" rel="noopener noreferrer" className="bg-[#0066CC] hover:bg-[#0052a3] text-white w-14 h-14 rounded-full shadow-lg hover:scale-110 transition duration-300 flex items-center justify-center border border-white/10 font-black text-sm" title="حراج">
          حراج
        </a>
      ...
      <section className="relative flex flex-col items-center justify-center text-center min-h-[95vh] px-4">
        <div className="absolute inset-0 z-0">
          <img src="/images/1.webp" alt="كارستايل لرش وزينة السيارات" className="w-full h-full object-cover brightness-50" />
          ...
          {[
            ...
            <div key={s.id} className="group bg-gray-800/40 backdrop-blur-sm rounded-[2rem] overflow-hidden border border-gray-700 hover:border-blue-500 transition-all duration-500 shadow-2xl">
              <div className="h-64 overflow-hidden">
                <img src={`/images/${s.id}.webp`} alt={`${s.title} - كارستايل`} className="w-full h-full object-cover group-hover:scale-110 transition-all duration-700" />
              </div>
            ...
            {[1, 2, 3].map((v) => (
              <div key={v} className="relative h-[500px] bg-gray-800 rounded-3xl overflow-hidden border border-gray-700 group">
                ...
                <div className="absolute bottom-6 right-6 text-right z-10">
                  <p className="text-white font-bold text-lg drop-shadow-md">لمسة إبداع {v}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <footer className="relative py-24 px-4 overflow-hidden text-center z-10 border-t border-gray-800">
        <div className="absolute inset-0 z-0">
          <img src="/images/5.webp" alt="موقع كارستايل الرياض" className="w-full h-full object-cover opacity-20" />
        ...
        <div className="relative z-10 max-w-6xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-16 items-start">
            ...
            <div className="text-right md:pr-10 border-r-0 md:border-r-2 border-blue-600 h-full flex flex-col justify-center py-4">
              ...
              <p className="text-gray-300 text-xl font-bold mb-2">السبت - الخميس: <span className="text-white">9:00 ص - 11:00 م</span></p>
              <p className="text-gray-300 text-xl font-bold">الجمعة: <span className="text-white">2:00 م - 11:00 م</span></p>
            </div>

            {/* الخريطة الجغرافية المصححة */}
            <div className="flex flex-col items-center justify-center w-full">
              <h4 className="text-blue-500 font-black mb-6 uppercase tracking-widest text-2xl italic">موقعنا الجغرافي</h4>
              <div className="relative w-full max-w-md h-64 bg-gray-800 rounded-2xl overflow-hidden border border-gray-700 shadow-2xl group">
                ...
                {/* زر الفتح المباشر في خرائط جوجل */}
                <a href={googleMapsLink} target="_blank" rel="noopener noreferrer" className="absolute top-3 right-3 z-20 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs py-2 px-3 rounded-md shadow-lg transition-all flex items-center gap-1 border border-white/10">
                  الفتح في الخرائط 📍
                </a>

                {/* iframe الخريطة التفاعلية */}
                {iframeSrc !== "ضع_رابط_التضمين_src_الخاص_بالمحل_هنا" ? (
                  <iframe title="موقع كارستايل" src={iframeSrc} className="w-full h-full border-0 opacity-90 group-hover:opacity-100 transition-opacity" allowFullScreen={true} loading="lazy" referrerPolicy="no-referrer-when-downgrade"></iframe>
                ) : (
                  <div className="w-full h-full flex items-center justify-center text-gray-400 p-4 text-center text-sm">يرجى إضافة رابط التضمين لإظهار الدبوس الأحمر</div>
                )}
              </div>
            </div>
